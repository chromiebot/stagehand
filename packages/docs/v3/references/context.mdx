...
<ParamField path="headers" type="Record<string, string>" required>
  An object containing header name-value pairs. Pass an empty object `{}` to clear
  all previously set extra headers.
</ParamField>

This method:
- Applies headers to all currently open pages immediately
- Automatically applies headers to any pages created after calling `setExtraHTTPHeaders()`
- Replaces any previously set headers when called again (headers are not merged)
- Useful for authentication tokens, custom tracking headers, or API versioning
...
// Navigate - headers will be sent with all requests
