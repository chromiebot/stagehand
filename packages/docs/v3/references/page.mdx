...
<ParamField path="headers" type="Record<string, string>" required>
  An object containing header name-value pairs. These headers are merged with (but take precedence over) headers set via other means.

  Pass an empty object `{}` to clear any previously set extra headers.
</ParamField>

This method:
- Sends the specified headers with every HTTP request made by the page
- Merges with existing headers (extra headers take precedence on conflict)
- Applies to all requests including XHR, fetch, and resource loading
- Persists until explicitly cleared or the page is closed

**Example - Set authentication headers:**
...
**Example - Set multiple headers for API access:**
...
```typescript
// Set custom headers for authenticated requests
await page.setExtraHTTPHeaders({
...
// All subsequent requests include the headers
await page.evaluate(async () => {
  // This fetch will include the Authorization header
  const response = await fetch("/api/protected");
  return response.json();
});

// Update headers for a different context
await page.setExtraHTTPHeaders({
